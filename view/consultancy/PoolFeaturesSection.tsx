...
import { poolFeatureItems } from "@/view/consultancy/pool-projects";

export function PoolFeaturesSection() {
  return (
    <section className="w-full px-2 pt-[50px] md:px-6 md:pt-[100px] lg:px-10">
      <Container>
        <Reveal>
          <SectionHeading className="mb-8">
            <span>Consultancy</span> Services.
          </SectionHeading>
        </Reveal>
        <Reveal delay={80}>
          <p className="mb-10 max-w-3xl leading-relaxed text-parkonic-primary">
            Complete property advisory — from buying and selling to valuation,
            legal review, and investment planning.
          </p>
        </Reveal>

        <div className="flex flex-wrap gap-6">
          {poolFeatureItems.map((item, index) => (
            <Reveal
              key={item.image}
              delay={index * 100}
